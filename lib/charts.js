...
function renderChartWithCallback(width,
  height,
  backgroundColor,
  devicePixelRatio,
  untrustedChart,
  ...
  if (typeof untrustedChart === 'string') {
    ...
    try {
      if (untrustedChart.match(/(for|while)\(/gi)) {
        errorBufferCallback('Input is not allowed', undefined);
        return;
        //return Promise.reject(new Error('Input is not allowed'));
      }
      const vm = new NodeVM();
  ...
  if (chart.type === 'sparkline') {
    if (chart.data.datasets.length > 1) {
      errorBufferCallback('"sparkline" only supports 1 line. Use "line" chart type for multiple lines.', undefined);
      return;
      // return Promise.reject(
      //   new Error('"sparkline" only supports 1 line. Use "line" chart type for multiple lines.'),
      // );
    ...
    if (chart.data.datasets.length < 1) {
      errorBufferCallback('"sparkline" requres 1 dataset', undefined);
      return;
      //return Promise.reject(new Error('"sparkline" requres 1 dataset'));
    }
    chart.type = 'line';
  ...
  try {
    getImageBuffer(width, height, chart, function (err, buffer) {
      if (err) {
        errorBufferCallback(err, undefined);
        return;
      } else {
        errorBufferCallback(undefined, buffer);
        return;
      }
    })
    //return canvasRenderService.renderToBuffer(chart);
    ...
    // canvasRenderService doesn't seem to be throwing errors correctly for
    // certain chart errors.
    errorBufferCallback(err.message, undefined);
    return;
    //return Promise.reject(err.message);
  }
}

async function getImageBuffer(width,height,chart,errorBufferCallback){
  try {
    //console.log(JSON.stringify(chart));
    const canvasRenderService = new ChartJSNodeCanvas({width, height});
    var image = await canvasRenderService.renderToBuffer(chart);
    errorBufferCallback(undefined,image);
  } catch (error) {
    errorBufferCallback(error,undefined);
  }
}
module.exports = {
  DEFAULT_COLORS,
